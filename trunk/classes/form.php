<?
	...
	class Form {		
        ...
        function GetDefaultValue($name, $type, $options) {
			...
			if ($this->frompersist) $name=$this->name.'.'.$name;
		...
		function Input($type, $name = NULL, $value = NULL, $options = array()) {
			...
			if (!is_null($value)) $options['value'] = htmlQuote($value);
			$options['type'] = $type;
		...
		function ControlHTML($type, $name, $value = NULL, $options=array()) {   
			...
			switch ($type) {
				...
				case FORM_DATE :
					...
					$y=intval(get($valarr,0,date("Y"))); $m=intval(get($valarr,1)); $d=intval(get($valarr,2));
						
					$ta_d=array(""=>""); for ($i=1; $i<=31; $i++) $ta_d[$i]=$i;
					$ta_m=array(""=>""); for ($i=1; $i<=12; $i++) $ta_m[$i]=$i;
					$ta_y=array(""=>""); for ($i=2007; $i <=date("Y"); $i++) $ta_y[$i]=$i;
					$opts_d = $options;
					$opts_m = $options;
					$opts_y = $options;
					$opts_d['_list'] = $ta_d;
		...
		function SubField($type, $name, $options = array(), $pattern = NULL) {
            ...
            if ($asdefault==AUTO) $asdefault=get_param($name.'__asdef',0);
            if ($value==AUTO) $value=$this->DefaultValue($name,$type,$options,$asdefault);
