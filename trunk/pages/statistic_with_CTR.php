		...
		<tr><td width=75% class='TableHeader' align='center'>Сайт</td><td width=20% align='center' class='TableHeader'>Показы сегодня</td><td width=5% align='center' class='TableHeader'>CTR</td></tr>";
		while ($row = mysql_fetch_row($set)) {
			if ($class == 'Table2') $class = 'Table1';
			else $class = 'Table2';
			$ctr = round($row[3]/$row[2]*100, 2);
			echo "<tr onmouseover=bg_on(this); onmouseout=\"bg_off(this, '');\" class='$class'><td><b><a href='http://$row[0]' target='_blank'>$row[0]</a></b> - $row[1]</td><td align=center>$row[2]</td><td align=center>$ctr</td></tr>";
		}
